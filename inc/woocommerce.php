 ...
 * @return void
 */
function astromag_woocomerce_setup() {
	add_theme_support( 'woocomerce' );
	add_theme_support( 'wc-product-gallery-zoom' );
	add_theme_support( 'wc-product-gallery-lightbox' );
	add_theme_support( 'wc-product-gallery-slider' );
}
add_action( 'after_setup_theme', 'astromag_woocomerce_setup' );
 ...
 */
function astromag_woocomerce_active_body_class( $classes ) {
	$classes[] = 'woocomerce-active';

	return $classes;
}
add_filter( 'body_class', 'astromag_woocomerce_active_body_class' );
 ...
 */
function astromag_woocomerce_products_per_page() {
	return 12;
}
add_filter( 'loop_shop_per_page', 'astromag_woocomerce_products_per_page' );
 ...
 */
function astromag_woocomerce_thumbnail_columns() {
	return 4;
}
add_filter( 'woocomerce_product_thumbnails_columns', 'astromag_woocomerce_thumbnail_columns' );
 ...
 */
function astromag_woocomerce_loop_columns() {
	return 3;
}
add_filter( 'loop_shop_columns', 'astromag_woocomerce_loop_columns' );
 ...
 */
function astromag_woocomerce_related_products_args( $args ) {
	$defaults = array(
		'posts_per_page' => 3,
		'columns'        => 3,
	);
...
add_action( 'woocomerce_before_main_content', 'astromag_woocomerce_wrapper_before' );

if ( ! function_exists( 'astromag_woocomerce_wrapper_after' ) ) {
	/**
	 * After Content.
	 *
	 ...
	 * @return void
	 */
	function astromag_woocomerce_wrapper_after() {
			?>
			</main><!-- #main -->
		</div><!-- #primary -->
		<?php
...
add_action( 'woocomerce_after_main_content', 'astromag_woocomerce_wrapper_after' );
 ...
 *
	<?php
		if ( function_exists( 'astromag_woocomerce_header_cart' ) ) {
			astromag_woocomerce_header_cart();
		}
	?>
 */
...
if ( ! function_exists( 'astromag_woocomerce_cart_link' ) ) {
	...
	function astromag_woocomerce_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'astromag' ); ?>">
			<?php
			$item_count_text = sprintf(
				/* translators: number of items in the mini cart. */
				_n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'astromag' ),
...
if ( ! function_exists( 'astromag_woocomerce_header_cart' ) ) {
	 ...
	 * @return void
	 */
	function astromag_woocomerce_header_cart() {
		if ( is_cart() ) {
		...
		<ul id="site-header-cart" class="site-header-cart">
			<li class="<?php echo esc_attr( $class ); ?>">
				<?php astromag_woocomerce_cart_link(); ?>
			</li>
			<li>
				<?php
				$instance = array(
